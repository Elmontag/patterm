...
export default function App() {
  ...
  const handleCreateSlot = async (event) => {
    ...
    const providerIdRaw = form.get("slot_provider");
    const providerId = (providerIdRaw ? providerIdRaw.toString().trim() : "") ||
      (user?.role === "provider" ? user.id : "");
    const departmentRaw = form.get("slot_department");
    const departmentId = departmentRaw ? departmentRaw.toString().trim() || undefined : undefined;
    const missing = validateRequiredFields([
    ...
    if (user?.role === "clinic_admin" && providers.length > 0 && !providerId) {
      setAdminFeedback("Bitte eine:n Behandler:in auswählen.");
      return;
    }
            ...
            {user?.role === "clinic_admin" && providers.length > 0 ? (
              <label className="text-xs font-semibold uppercase text-slate-500">
                ...
                <select
                  ...
                  className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2"
                >
                  <option value="" disabled={providers.length === 1}>
                    Bitte auswählen
                  </option>
